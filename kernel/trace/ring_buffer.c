...
extern unsigned long trace_functions_called2;
				...
				struct ring_buffer_event *event)
{
	...
	if (likely(rb_try_to_discard(cpu_buffer, event))) {
		...
		/*
		 * if another write came in between the try_to_discard() and
		 * the set_commit_to_write(), but was not committed yet,
		 * we just committed it  - possibly corrupting the buffer.
		 * ...Oops!
		 */
	}
}
		...
		struct ring_buffer_event *event, int length)
{
	...
	/* discard if event type != DATA */
	if (prev_event->type_len > RINGBUF_TYPE_DATA_TYPE_LEN_MAX)
		return 0;

	/* discard if prev_event < buffer start */
